...
//int dx[]= {-1, 1,-1, 1, 1, 0,-1, 0};/*8 side move*/
//int dy[]= {-1,-1, 1, 1, 0, 1, 0,-1};/*8 side move*/
//int dx[]={1, 1,-1,-1, 2,-2, 2,-2};/*knight move*/
//int dy[]={2,-2, 2,-2, 1, 1,-1,-1};/*knight move*/
...
void dfs(int src, int callNo)
{
    vis[src] = 1;
    for(int i=0;i<adj[src].size();i++)
    {
        if(vis[adj[src][i]]==0)
            dfs(adj[src][i],callNo);
    }
    if(callNo)
        srcList.push(src);
}
...
//    fWrite("out");
    ...
    for(int t=1;t<=T;t++)
    {
        ...
        while(!srcList.empty())srcList.pop();
        int n,edge,u,v,result=0;
        cin>>n>>edge;
        ...
        callNo = 0;
        MEM(vis,0);
//      int len =(int)srcList.size()
//      for(int i=1;i<=;i++)
        while(!srcList.empty())/// Select the Original sources
        {
            int to = srcList.top();
            srcList.pop();
            if(!vis[to])
